...
router.post('/challenges/:id/accept', requireAuth, async (req, res, next) => {
  try {
    ...
    // Atomically flip the challenge to accepted. If a second concurrent accept
    // beats us here, our UPDATE matches 0 rows and we roll back our orphan game.
    const claimed = await sql`
      UPDATE challenges
      SET status = 'accepted', game_id = ${gameId}, updated_at = NOW()
...
router.post('/games/:id/move', requireAuth, async (req, res, next) => {
  try {
    ...
    // Atomic optimistic write: the UPDATE only fires if num_moves/turn/status
    // still match what we read, and the INSERT only fires if the UPDATE matched.
    // A concurrent move loses the race and gets 0 rows back.
    const claimed = await sql`
      WITH upd AS (
        UPDATE games
    ...
    `;
    if (!claimed.length){
      return res.status(409).json({ error: 'Game state changed — refresh and try again' });
    }

    // If the game just ended, write one matches row per player so it counts
    // toward stats + leaderboard. Best-effort: a failure here shouldn't
    // invalidate the move that just succeeded.
    if (status !== 'active'){
      try {
        await recordOnlineMatches({
        ...
        console.error('recordOnlineMatches failed', e);
      }
    }
  ...
  try {
    ...
    try {
      await recordOnlineMatches({
        ...
        blackUsername: g.black_username,
        status: 'resigned',
        endingType: 'resignation',
        winnerId,
        numMoves: g.num_moves,
        createdAt: g.created_at
      });
    } catch (e){
      console.error('recordOnlineMatches failed', e);
...
// Insert one matches row per player when an online game terminates, so
// online play feeds into the same stats + leaderboard as AR/computer games.
async function recordOnlineMatches({
  whiteId, blackId, whiteUsername, blackUsername,
  status, endingType, winnerId, numMoves, createdAt
}){
  ...
  if (status === 'checkmate' || status === 'resigned'){
    whiteResult = winnerId === whiteId ? 'win' : 'loss';
    blackResult = winnerId === blackId ? 'win' : 'loss';
  } else {
    whiteResult = 'draw';
    blackResult = 'draw';
  }
  await sql`
    INSERT INTO matches
      (user_id, result, user_color, opponent, ending_type, num_moves, duration_seconds)
    VALUES
      (${whiteId}, ${whiteResult}, 'w', ${blackUsername}, ${endingType}, ${numMoves}, ${durationSec}),
      (${blackId}, ${blackResult}, 'b', ${whiteUsername}, ${endingType}, ${numMoves}, ${durationSec})
  `;
}
